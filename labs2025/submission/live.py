...
def change_state():
    ...
    if cur_state == State.start:
        ...
        for marker in markers:
            # Βάζει το χρώμα που έχει το marker που δείχνει αριστερά, στην αρχή της προτεραιότητας
            ...
            elif marker.get_orientation() == ar_solver.Orientation.RIGHT:
        ...
        for marker in markers:
            # Βρίσκουμε την απόστασή μας από το marker
            ...
            # Αν έχουμε το marker 1 ΚΑΙ είμαστε κοντά στο marker
            if int(marker.get_id()) == 1 and 0 < marker_distance < 200:
                # Επιστρέφουμε την προτεραιότητα στο κανονικό
                colors = [PURPLE, ORANGE, RED, GREEN, BLUE]
                colors_copy = [PURPLE, ORANGE, RED, GREEN, BLUE]
                # Βάζουμε πρώτα στην προτεταιότητα το χρώμα που ΔΕΝ υπήρχε στα AR markers
                for color in colors:
        ...
        for marker in markers:
            ...
            if marker.get_color() == "PURPLE":
                ...
                break
            # Για να πάμε από το lane following στο cone slaloming
                ...
                # και βρίσκουμε την απόσταση του marker από εμάς
                marker_distance = rc_utils.get_pixel_average_distance(depth_image, marker_center)
                # Αν η απόσταση του marker από εμάς είναι σχετικά μικρή
...
def follow_lane(max_speed=0.25):
    ...
    if HARD_TURN == True or (len(contours_secondary) == 1 and len(contours_primary) <= 1):
        ...
        if not HARD_TURN:
            ...
            hard_turn_counter += 1
            # Στην πρώτη απότομη στροφή πάμε αριστερά
            if hard_turn_counter == 1:
                angle = -ONE_LANE_TURN_ANGLE
            # Σε όλες τις υπόλοιπες πάμε δεξιά
            elif hard_turn_counter == 2:
                angle = ONE_LANE_TURN_ANGLE
            else:
                angle = -ONE_LANE_TURN_ANGLE
            color = None
...
# Για να βρίσκουμε τους κώνους, χρησιμοποιείται από τη cone_slalom2()    
def find_cones():
    """
...
# Στην πραγματικότητα είναι lane following με κώνους
...
# Κατά τα πάσα πιθανότητα δεν θα χρειαστεί
def follow_walls(max_speed, sensitivity):
    ...
    if len(markers) == 1 and int(markers[0].get_id()) == 199:
        ...
        if 0 < marker_distance < 70:
            speed = 0.3
            if markers[0].get_orientation() == ar_solver.Orientation.RIGHT:
